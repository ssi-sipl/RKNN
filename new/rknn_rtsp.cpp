 ...
 while(running){
   ...
   if(avcodec_send_packet(ctx,pkt)==0){
      while(avcodec_receive_frame(ctx,frame)==0){
         ...
         av_frame_ref(shared_frame,bgr_frame);
         frame_ready=true;
         SDL_UnlockMutex(frame_mutex);

         SDL_LockMutex(infer_mutex);
         av_frame_unref(infer_frame);
         av_frame_ref(infer_frame,bgr_frame);
         infer_ready=true;
...
void inference_loop(){
 ...
 while(running){
   if(!infer_ready){sleep_ms(1);continue;}

   SDL_LockMutex(infer_mutex);
   if(!infer_frame || !infer_frame->data[0]){
      SDL_UnlockMutex(infer_mutex);
      continue;
   }

   av_frame_unref(local);
   av_frame_ref(local,infer_frame);
   SDL_UnlockMutex(infer_mutex);

   cv::Mat bgr(video_h,video_w,CV_8UC3,
               local->data[0],
               local->linesize[0]);
   ...
   cv::Mat input;
   cv::resize(bgr,input,cv::Size(640,640));
   ...
   std::vector<Detection> local_det;
   ...
   for(int i=0;i<8400;i++){
      ...
      local_det.push_back(d);
   }

   SDL_LockMutex(det_mutex);
   detections=local_det;
